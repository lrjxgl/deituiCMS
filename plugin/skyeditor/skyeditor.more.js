...
skyeditor.html='<div class="sky-editor-loading">上传中...</div><div class="sky-editor">	<div class="sky-editor-colors">			</div>	<div class="sky-editor-emojis">			</div>	<div class="sky-editor-tools">		</div>	<input type="file" class="sky-editor-file" name="upimg" multiple="multiple" style="display: none;" />	<input type="file" class="sky-editor-file-video" name="upimg" style="display: none;" /></div>';
...
skyeditor.init = function () {
   	if(skyeditor.isfirst){ 
		...
		var els=$(".sky-editor-content");
		for(var i=0;i<els.length;i++){
			$(".sky-editor-content:eq("+i+")").after(skyeditor.html);
			$(".sky-editor-content:eq("+i+")").after('<textarea class="sky-editor-textarea" style="display:none;" name="'+$(".sky-editor-content:eq("+i+")").attr("name")+'"></textarea>');
		}
		...
		setInterval(function(){
			var els=$(".sky-editor-content");
			for(var i=0;i<els.length;i++){
				$(".sky-editor-textarea:eq("+i+")").val($(".sky-editor-content:eq("+i+")").html());			
			}
		},600);
...
$(function () {
    ...
    $(document).on("click", ".sky-editor-emoji-toggle", function () {
		var $parent=$(this).parents(".sky-editor");
        $parent.find(".sky-editor-emojis").toggle();
    })
    ...
    $(document).on("click", ".sky-editor-colors-toggle", function () {
		var $parent=$(this).parents(".sky-editor");
        $parent.find(".sky-editor-colors").toggle();
    })
    $(document).on("click", ".sky-editor-img", function () {
		
		var $parent=$(this).parents(".sky-editor");
        $parent.find(".sky-editor-file").click();
    })
    $(document).on("click", ".sky-editor-video", function () {
		var $parent=$(this).parents(".sky-editor");
        $parent.find(".sky-editor-file-video").click();
    })
    ...
    $(document).on("click", ".sky-editor-content", function () {
		var $parent=$(this).parents(".sky-editor");
        if ($parent.find(".sky-editor-content div").length == 0) {
            $parent.find(".sky-editor-content").prepend('<div style="color:#555;">&nbsp;</div>')
        }
        skyeditor.skyEl = $(this).find("div").eq(0);

    })
    $(document).on("click", ".sky-editor-content div,.sky-editor-content p", function (e) {
        e.stopPropagation();
		var $parent=$(this).parents(".sky-editor");
        skyeditor.skyEl = $(this);
        var color =  $parent.find(".sky-editor-color").css("background-color");
