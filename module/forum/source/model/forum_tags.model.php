<?php
class forum_tagsModel extends model{
	public $table="mod_forum_tags";
	...
	public function Done($objectid,$tags){
		...
		$indexlist=M("mod_forum_tags_index")->selectCols(array(
		...
		if($deleteIndex){
			...
			M("mod_forum_tags_index")->delete("objectid=".$objectid." AND tagid in("._implode($delTagsId).") ");
		...
		if(!empty($insertIndex)){
			M("mod_forum_tags_index")->insertMore($insertIndex);
		}
		 
	}
	
	public function getForumByKey($gkey){
		...
		$data=M("mod_forum_tags_index")->select(array(
